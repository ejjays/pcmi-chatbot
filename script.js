...
function createWaveEffect(isDark) {
  // Create container for wave effect
  const waveContainer = document.createElement('div');
  waveContainer.className = 'wave-container';
  
  // Create wave element
  const wave = document.createElement('div');
  wave.className = `wave ${isDark ? 'dark-wave' : 'light-wave'}`;
  
  // Add wave to container
  waveContainer.appendChild(wave);
  document.body.appendChild(waveContainer);
  
  // Remove wave container after animation
  wave.addEventListener('animationend', () => {
    document.body.removeChild(waveContainer);
  });
}

// Modify your existing theme toggle handler
document.getElementById('theme-toggle-button').addEventListener('click', () => {
  const isDarkMode = document.body.classList.contains('dark-mode');
  createWaveEffect(!isDarkMode);
  
  // Toggle dark mode class after a small delay to sync with animation
  setTimeout(() => {
    document.body.classList.toggle('dark-mode');
    // Update toggle button icon if needed
    const themeButton = document.getElementById('theme-toggle-button');
    themeButton.textContent = isDarkMode ? 'light_mode' : 'dark_mode';
  }, 500);
});
